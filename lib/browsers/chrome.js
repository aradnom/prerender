...
chrome.setUpEvents = function(tab) {
	return new Promise((resolve, reject) => {
		...
		Promise.all([
			...
			// If request blacklist is defined, set that here
			// Any XHR URLs in this list will be automatically blocked on
			// * matching is supported
			if (this.options.requestBlacklist && this.options.requestBlacklist.length) {
				Network.setBlockedURLs({ urls: this.options.requestBlacklist });
			}
